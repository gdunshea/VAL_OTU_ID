...
author: "Glenn Dunshea - Using Claude AI for python script refinement"
...
3. **Reference database**: A reference database FASTA file in dada2 format
...
| `*_database_validated.csv` | Ref DB sequence comparison results |
...
2. **Indexes all species of that genus** from Ref. DB
...
| **UNCERTAIN** | Some regional congeners missing from Ref. DB |
| **REASSIGN** | A congener matches >0.5% better than assigned |
| **NO_REF** | Assigned species not in Ref. DB |
...
This is where both the geographic plausibility information and database completeness evidence aere combined to make a final decision.
...
**Legend:** Ref. DB = sequence reference database checks. GBIF/OBIS = geographic occurrence checks. Green = confident ID. Yellow = dropped to higher rank.
...
- A better-matching congener exists in Ref. DB but is **NOT geographically local**
...
digraph cf_notation {
  ...
  match [label='REF. DB: Seq match ≥ cf_threshold_pct?', fillcolor='#FFEAA7', shape=diamond, width=2.8]
...
| `--midori`, `-m` | Ref. DB reference FASTA (filename or full path) |
...
| `--reference-dir` | `Reference_databases` | Directory for Ref. DB when filename only given |
...
The sequence identity thresholds should be adjusted based on your amplicon length. Use this table to set `--reassign-diff-pct` based on how many nucleotide differences you want to require for species reassignment.

### Nucleotide Differences as Percent Identity
...
Here is an example command with some custom minimum percentages for acceptable species, genus and family level ID. Also, distance-thresholds for plausible IDs and congener search thresholds. There are status updates of the script progress in the terminal until the script is complete. 
...
When the script is finished a summary of results is provided in the terminal and all the output files can be found in the "results_files" directory of the repo. These results can be easily incorporated into your existing phyloseq object by substituting your old phyloseq "tax_table" with the results file with the suffix "phyloseq". This table has all the extra fields that VAL_OTU_ID adds that describe the validation process and results, species of concern status, etc. 
...
| `best_db_match` | Best matching species in Ref. DB |
...
4. Check if Ref. DB has sequence errors for that species
